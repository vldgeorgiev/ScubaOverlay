...
Enable ScubaOverlay to generate overlay videos for specific segments of a dive that match camera video clips, rather than always generating overlays for the entire dive. This allows divers who record multiple short videos during a single dive to create accurate overlays for each clip with automatic timezone offset detection. Uses OpenCV (already a project dependency) for video metadata extraction.
